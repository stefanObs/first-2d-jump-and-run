...
Builtin **Outlaw Cave** (level 5) and cave arc **levels 11–16** (`Crystal Mouth` → `Dragon Gate`) use `"style": "cave"`. Levels 11–16 all place pink ceiling drops, spring pads, and a trail of sheriff badges; levels 12–16 include conveyor belts (no gates — those stay a desert prop); all six place extra ladders/planks and decorative fences. **Wing Chasm** (level 15) is the cave flying trail: it stamps `wings` at camp before the first badge, plus two more pairs mid-trail, and hangs badges in the upper cave air; planks over both chasms keep it beatable on foot. After Dragon Gate the **Cave Dragon** boss starts on the floor, takes off when the fight begins, flies left↔right below the ceiling (facing travel direction, wing-flap + landing poses; after each lasso the flying frames keep the same neck / neck+torso rope layout as the landed tied stages), spits a few flameballs in a straight line toward the cowboy (2 rounds × 2 shots; shots end on the floor), lands for a lasso, then repeats; after three lassos his mouth is tied and the campaign victory plays. Workshop imports for 11–16 use the stamp catalog in `CaveCampaignLevels`. Custom trails can pick Cave style freely.
