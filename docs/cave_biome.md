...
4. **Cave wash backdrop** — no sun/mesas; deep `cave_sky` wash that meets the trail floor + high hanging rock ceiling (`cave_ceiling_tile`) with sparse droppable stalactite teeth fused under the band (**Dragon Gate** and the **Cave Dragon** arena keep a clean rock band with no teeth). Rock band is solid (`FlightCeilingCave`); touching it while flying respawns at camp.
