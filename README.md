...
Child-friendly 2D western cowboy platformer (Godot **4.4**). Aimed at kids ~6: forgiving jumps, nonviolent lasso. **Classic (simple) mode is the default** for every start and continue — no lives or game over. **Advanced Mode** (hearts) is opt-in: cycle the hearts button on the start screen, or pick a row in **Settings**, before starting or continuing. It has four badge paces — **★5 / ★10 / ★15 / ★30** sheriff badges for each extra life. ★5 and ★10 start with **5 hearts**; ★15 and ★30 start with **3**. The hearts and sheriff star fill that start-screen button so the ★ number stays readable. Pick **Cowboy** or **Cowgirl** on the start screen (or in Settings) — each **save slot** stores its own rider; focusing a filled door restores that rider, not the trail mode. Starting/continuing commits the current title-screen picks (rider + Classic/Advanced) to that slot. **German is the default language**; English is fully supported.

**Content version:** `1.8.97` (see `content_version.txt`). Launchers reimport when this stamp changes.
...
11. Crystal Mouth — cave remaps, ladder path splits, springs, ranch fences, first pink drops; three lantern camps  
12. Bat Gallery — bats, drips, ledges, springs, first cave conveyor gate; three lantern camps  
13. Acid Veins — pink drip gauntlet, fungus, canyon hop with approach spring, conveyor gate; three lantern camps  
14. Ladder Grotto — ladder branches, lizards, springs, reverse belt, ceiling drips; three lantern camps  
...
16. Dragon Gate — cave finale with belts, ladders, springs, drips, and fences before the Cave Dragon; three lantern camps  
...
**Campaign Workshop / trail editor:** Upper half edits the trail with a **level style** picker (**Desert** / **Cave** / **Horse**), a **category picker** (each category shows an example thumbnail) plus stamp tools — the **Trail / path** category uses direct **Dirt / Canyon / Plank** icon buttons (canyon icon: both ridges with sky-blue gap; cave style swaps labels to Cave Floor / Cave Gap / Crystal Ledge) plus **Conveyor** and **Fence** stamps (both styles) and **Timed Gate** (desert only — caves have no gates), while other categories use a tool dropdown. Stamp category, tool picker, thumbnails, and path-stamp buttons match the **action-button** height so they are easy to tap. A **Motion** category stamps **Moving Plank**, **Moving Cloud**, **Blink Cloud**, and **Wind** (campaign canyon/gust props). Cave ceiling height/panels stay automated (not stamped). Cave style remaps stamp presentation (poison fungus, bow skeletons, cave lizards, scorpions, lantern camps, Crystal Gate) and unlocks cave-only stamps (pink ceiling drops, falling stalactites, curve-flying bats). **Horse** is a desert play theme with `start_mounted` (cowboy begins on horseback); treasure chests and power-up item stamps (**Wings / Magic Boots / Speed Star / Bubble Shield**) are banned and stripped on save. See `docs/cave_biome.md`. **+ Length / − Length** adjust the trail in **12-column steps** (defaults match built-in campaign width: **180 columns**; min 12, max 180). A stamp grid shows the **full level height** plus a horizontal slide bar to reach the end; a **▼ / ▶ chevron** collapses or expands the grid for the session so the live preview gets more room; collapsing **shrinks the grid pane** so status, pan buttons, and preview **rise** instead of sitting under an empty hole. **Right-click** a grid cell or the live preview to remove the element there. Sideways grid scroll is **edge auto-scroll or the slide bar only** (no wheel/trackpad pan). **Ground-standing props** (cactus/fungus, camps, springs, bandits/skeletons, trail bulls/lizards, ninjas, snakes/scorpions, power-ups, treasure chests, conveyors, timed gates, fences, saloon/Crystal Gate) stamp **one row above dirt** so they stand on the trail surface and need **solid dirt under every trail column of their footprint** — never on a **canyon** or **pit** mouth (editor rejects, sanitize/build strip, layout rules reject campaign slips). **Camps** always sit on the **top of stacked dirt** in their column (raising the bank lifts the camp so respawn is never inside the floor). Non-ground stamps **do not stack**: placing one clears any overlapping non-ground footprint first (**ladders are the exception** — the climb shaft may share cells with the upper ledge and stamps on that ledge, so you can plant several ladders and still put a plank or badge above them). **Walking enemies** (bandit/skeleton, bull/lizard, ninja, snake/scorpion) may stand on a **plank / crystal ledge** without wiping it; the live preview shows them on that ledge. Ceiling drips/stalactites hang from the top rows; hovering shows a ghost at **final in-game size** of where the stamp will land, and the live preview draws a **grid-aligned cell outline** plus a footprint at the correct world size (plank 160×32, pit 128×64, etc.). Hovering an **already-placed** stamp outlines that same footprint (grid + live preview) so you can see what **right-click** will remove. **Bounty Bandit**, **Trail Bull**, **Ninja**, and **Carrion Bird** stamps join the enemy palette (cave: Crystal Skeleton / Cave Lizard / Cave Bat instead of bird). Clicking the **live preview** places the selected stamp at the matching column/row (same as the grid). Lower half gives priority to a **live gameplay preview** (same build/theme as play) framed to the **full vertical extent**; horizontal pan uses the same **edge auto-scroll or slide bar** as the grid (no mid-pane follow). Chrome and grid heights stay compact so the preview stays visible on 720p. Explicit **Save Trail** and **Reset Changes**. Top chrome holds **Back** (left-arrow icon), **Save**, **Reset**, **Export/Import**, **Play Test**, and **± Length** — the length buttons stay on-screen (they wrap under the action buttons if the row is too wide). No second Back under the grid. Action chrome uses the same kid-readable **picture icons** as the workshop hub so buttons stay clear without reading. Scroll arrows stay icon-only. The grid keeps a minimum edit height and scrolls vertically when the window is short so stamps stay tappable. Adjacent **canyon stamps merge into one wider gap** at build time. The live preview **keeps its pan** when you stamp or raise dirt (no smoothed camera slide after a rebuild). Each merged canyon shows **grow/shrink arrows** on both lips (same wood hover-pop and click-squash as other menu buttons). A **too-wide** warning icon appears when the gap is bigger than a normal jump (Horse theme uses the mounted jump reach). **Export Trails / Import Trails** in the workshop (or **Export Trail / Import Trail** in the editor) share custom levels as one portable `.cowboytrail` JSON pack — the editor reads/writes a single trail into the current session without auto-saving until you press Save. Translation editor (debug-only, F1 on save select) edits DE/EN CSV export into `savegames/`.
...
- **Move:** arrows or A/D; **jump:** Space (coyote ~0.16s, buffer ~0.15s, variable height). On a **ladder**, Space / W / Up climbs up and S / Down climbs down (left/right steps off). At the **top**, Space jumps so the cowboy can hop onto a nearby plank, mover, or badge. Xbox: stick/D-pad, A jump/climb-up, X lasso, B back, Menu pause. Pause offers **Restart Level** (reload the open trail from its start camp; clears that level’s mid-run camp save) and, on campaign saves, **Restart Trail at Level 1**, plus **Quit / Beenden** to leave the game.
...
- **Lasso:** Alt / F / L (Xbox X) — ties bandits, trail bulls, and ninjas (pass-through, seated rope pose). A downward jump stomp from above also ties + small bounce; side/upward/standing contact hurts. Trail bulls **charge the cowboy** when he is nearby. **Ninjas ambush** from their stamp, then slash with a sword; while the cowboy **flies with Wings**, they throw handcrafted **shuriken**. Bandits placed in mid-air (workshop or level layout) **fall to the walkable surface below** before patrolling; bandits stamped **inside or under the dirt** are **lifted onto the trail crust** (not onto movers overhead).
...
- **Player characters:** **Cowboy** or **Cowgirl** per save slot (title-screen mascots or Settings). Focusing a filled door restores that slot’s rider, not the trail mode. Starting the slot writes the current title-screen picks onto it. Both share the same animation set (idle, run, jump, climb, celebrate, Magic Boots variants). **Idle uses a single pose frame** with a gentle scale breathe — do not alternate mismatched idle frames (causes flicker). Title-screen mascot selection uses scale + star mark only (no red selection border).
- **Canyons vs cactus vs pits:** **Canyons** are wide trail gaps (merged stamps, hand-painted rims, sky mouth). **Pits** are fixed **128×64 px** holes (`assets/world/pit.png`, no scaling) stamped only on trail **dirt** cells. Falling into a pit or canyon → spin recovery → camp. Canyon spin/respawn starts only once the cowboy is **below the lower of the two bank lips** (a jump or fly-through still above that lower rim does not trigger). Cactus/bandit/carrion/snake hurt → camp (Bubble can block some damage). User-facing copy: call wide gaps **canyons**; call fixed holes **pits** (never use “pit” for a canyon).
...
- Bandits: downward jump stomp from above / lasso tie; any other contact hurts; **patrol planks / crystal ledges** and **turn at the lips** (do not walk off). **Trail bulls / cave lizards** and **ninjas** do the same while walking or charging — they move on the ledge and turn at the end instead of falling. Rattlesnakes / scorpions stay planted.
- **Trail bulls / cave lizards:** trail bulls use ring-free standing art plus a short run cycle while charging (cave lizards keep lizard art); **charge toward the cowboy** when he is nearby and **keep that heading** until a **pit or canyon lip** (then turn inland and keep running) — they do not retarget every frame, so they no longer look like they flee first; when the cowboy **jumps over** them on the same bank they keep going about **5 cm** before turning to chase; workshop/campaign placement keeps them off **pit mouths and canyon stamps** (they may stand and charge on **planks / crystal ledges**, turning at the lips); lasso or head stomp plays the full boss tying sequence (rope coils → legs bound → tip over → lying on the floor at the same on-screen size); side contact hurts. Stampede Bull boss keeps the painted back ring and uses its own run frames while charging — framed on a roomy 440×230 canvas (clear margin around horns, tail, and ring glow), with each run pose normalized to the standing pose's visible painted mass so tucked strides do not shrink. Wall and lasso reactions recoil/rotate without scaling the bull.
- **Ninjas:** handcrafted **chibi cel-shaded sprites** in the same big-head / thick-ink style as the bandit (`tools/build_ninja_frames.py`: hooded head, red headband + obi, navy gi, 64×80); workshop stamp marks an **ambush anchor** — the live editor preview shows the idle ninja at that stamp so you can see where he will trigger, while **in play** he stays hidden until the cowboy enters range, then **appears at that stamp** and runs in with a **sword slash** on reach, **jumps pits and canyons** toward the cowboy (crouch + airborne jump frames), and throws hand-drawn **shuriken** at winged flyers; lasso or head stomp ties with a unique bound rope pose **seated on the dirt or plank under him** (never left hanging in a jump); **camp respawn** cancels appear/chase/jump and hides him back at the stamp post (unless already tied at that camp). Only one animated body is drawn (`NinjaSprite`) — never leave a second scene `Sprite2D` behind, or he can look doubled with each half facing the other way.
...
- Where desert banks sit at different heights **with continuous ground** (no canyon between), paint a **natural soft curved slope** (gentle dune) with trail desert/dirt art (not a flat cliff face or ColorRect step). Slopes must be **walkable without jumping** (carve away Ground cliff walls into the dune; long gentle run so peak grade stays walkable). Slope crust ends must **start and end on the flat desert tops** (same height/seam, no stepped lip). Under the curved crust, pack **dense earth fill** with a solid warm `FloorSlopeUnderfill` polygon (dirt texture, not thousands of dirt sprites) that continues **far below** the dune — **no black or sky gaps** below the bank face. Flat banks use one `FloorDirt` seam row plus a textured `FloorAbyss` polygon for the deep earth. Do not stretch a dune across a short run when the grade would look like a dirt volcano — keep a packed-earth bank instead. **Narrow 1–2 column dirt spikes** much taller than their neighbors ease down to a short walkable bank (at most two cells above the lower neighbor). Raise a **wider plateau** to keep a tall hill.
...
- Every campaign trail has **at least 3 camps** (desert flags / cave lanterns) on solid dirt, spaced along the run — never on a canyon or pit mouth.
...
- Start screen, settings, pause, save select, **Campaign Workshop**, and **Advanced Mode game over**: stay **handcrafted** and trail-themed (polish may continue; do not regress to stock Godot chrome or mismatched stock art). Save select is **kid-first** and matches the painted proposal look: full desert-sunset backdrop, hat title logo, clickable cowboy/cowgirl mascots (chosen rider is highlighted and sets the next run’s character), three arched wooden **1 / 2 / 3** doors (horseshoe when empty, sharp character portrait + star dots when filled), left-hinged wood leaf that opens ~36° on hover/focus so desert/cave trail peeks through the free right edge (cowboy/cowgirl portrait in the porthole stays round, not squashed), circular wood chrome for settings / hearts / workshop, a wood **Exit Game / Spiel beenden** button (door-and-arrow icon) to leave the game, styled delete-save confirm, and an F1 **debug strip**. Settings and pause use warm wood panels with cream-on-wood buttons. **Menu buttons pop on hover and squash on click** (brighter wood hover, darker inset pressed tone, short tap SFX) — including start-screen chrome, Settings, pause, workshop hub, trail-editor actions, and translation-editor chrome. Start-screen save doors use the ajar swing instead of a hover pop. Campaign Workshop matches the start-screen desert chrome (compact hammer title, circular back button) with **picture icons on every action button** so kids can tell Edit / Add / Remove / Export at a glance — not a giant saloon title board. The trail editor reuses that icon language on Back / Save / Reset / Export / Import / Play Test / ± Length and scroll arrows. Game over reuses that desert backdrop and a saloon title board (no flat black/red wash). Prefer picture-first doors over dense saloon text boards or pointing-hand motifs. Boot loading uses painted saloon boards plus a **warm wood/gold progress bar** under “Saddling up”.
...
7. Bandits: stomp/lasso tie; side hurt; patrol planks and turn at the lips. Walking enemies (bandits/skeletons, bulls/lizards, ninjas) move on planks but must not walk off them.
...
11. **Dune slopes:** dense earth under the crust continuing far below the bank (`FloorSlopeUnderfill` / `FloorAbyss` polygons, not a dirt-sprite spray); no black/sky holes below the curved face.
12. **Workshop editor:** placement ghosts and preview icons use **final in-game stamp size** (`CustomLevelStore.stamp_visual_world_rect`) — planks / crystal ledges are **160×32** (4 cells), matching campaign `Block` planks; right-click removes stamps; hovering a placed stamp outlines its final footprint; **▼ / ▶** chevron collapses the stamp grid for the session (chrome and preview **rise**; no empty hole); stamps do not overlap (ladder climbs may share cells with the upper ledge; walking enemies may stand on planks and patrol them without walking off); ground-standing stamps never sit on canyon/pit mouths; **saloon / Crystal Gate** stamps only on the trail floor (not planks or air); **camps sit on the top of stacked dirt** (never buried so respawn is inside the floor); **narrow 1–2 column dirt spikes** ease to a short walkable bank (raise a wider plateau for taller hills); hub + trail-editor action buttons keep **kid-readable picture icons**; live preview **does not slide** after stamps or dirt-height rebuilds; merged canyons keep **grow/shrink lip arrows** (hover pop / click squash) and a **too-wide jump** warning icon; ninja stamps stay visible as idle markers in the live preview only (`editor_marker`), not in play.
