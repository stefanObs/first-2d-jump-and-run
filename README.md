...
**Content version:** `1.8.69` (see `content_version.txt`). Launchers reimport when this stamp changes.
...
**Saves:** three slots; auto-save; local `savegames/` (gitignored). `SAVE_VERSION` 4 — older formats discarded. Delete via card context / Space / Xbox Y + confirm. The **Campaign Workshop** list shows trails in real campaign order: **self-made** extras sit before the campaign level they target (or at the end), **changed** built-ins are marked, and **Add before** works on every row including self-made trails.
...
- Start screen, settings, pause, save select: stay **handcrafted** and trail-themed (polish may continue; do not regress to stock Godot chrome or mismatched stock art). Save select is **kid-first**: desert skyline stage, hat emblem + short title, cowboy/cowgirl mascots, three giant numbered **1 / 2 / 3** wood doors (portrait + capped star dots when filled), icon chrome for settings / hearts trail-mode / Campaign Workshop, and an F1 **debug strip** (Element Names + Translation Editor). Prefer picture-first doors over dense saloon text boards or pointing-hand motifs. Boot loading may still use painted saloon boards.
