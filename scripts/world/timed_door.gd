...
func _setup_fence_visual() -> void:
	...
	if _left_gate == null:
		...
		_left_gate.offset = Vector2(24, 0)
		_left_gate.position = Vector2(-24, 0)
		_left_gate.scale = Vector2(0.72, 0.78)
	...
	if _right_gate == null:
		...
		_right_gate.offset = Vector2(-24, 0)
		_right_gate.position = Vector2(24, 0)
		_right_gate.scale = Vector2(-0.72, 0.78)
		_visual.add_child(_right_gate)
	if _shape != null:
		var rect := RectangleShape2D.new()
		rect.size = Vector2(56, 100)
		_shape.shape = rect
		_shape.position = Vector2(0, -50)
...
func _snap_gates() -> void:
	if _left_gate == null or _right_gate == null:
		return
	if _open:
		_left_gate.rotation_degrees = -78.0
		_right_gate.rotation_degrees = 78.0
		_visual.modulate = Color(1, 1, 1, 0.85)
	else:
		_left_gate.rotation_degrees = 0.0
		_right_gate.rotation_degrees = 0.0
		_visual.modulate = Color(1, 1, 1, 1)


func _animate_gates() -> void:
	if _left_gate == null or _right_gate == null or _animating:
		_snap_gates()
		return
	_animating = true
	var left_rot := -78.0 if _open else 0.0
	var right_rot := 78.0 if _open else 0.0
	var tween := create_tween()
	tween.set_parallel(true)
	tween.tween_property(_left_gate, "rotation_degrees", left_rot, 0.28).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	tween.tween_property(_right_gate, "rotation_degrees", right_rot, 0.28).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	tween.tween_property(_visual, "modulate:a", 0.85 if _open else 1.0, 0.2)
...
func _update_warning_blink() -> void:
	if _visual == null:
		return
	if _timer > warn_time:
		_visual.modulate = Color(1, 1, 1, 0.85 if _open else 1.0)
		if _label != null:
			_label.text = "OPEN" if _open else "FENCE"
		return
	if not _warned:
		_warned = true
		first_warn.emit()
	var pulse := 0.45 + absf(sin(_blink_phase)) * 0.55
