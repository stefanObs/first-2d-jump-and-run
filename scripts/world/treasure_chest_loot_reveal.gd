...
func play(loot: TreasureChestLoot.Type, target_world: Vector2) -> void:
	...
	_sparkle.scale = Vector2(0.85, 0.85)
	...
	tween.parallel().tween_property(_sprite, "scale", Vector2(1.35, 1.35), POP_DURATION)
	tween.parallel().tween_property(_glow, "scale", Vector2(1.85, 1.85), POP_DURATION)
	tween.parallel().tween_property(_glow, "modulate:a", 0.9, POP_DURATION * 0.7)
	tween.parallel().tween_property(self, "position:y", position.y - RISE_DISTANCE, POP_DURATION)
	tween.parallel().tween_property(_sparkle, "modulate:a", 1.0, POP_DURATION * 0.45)
	# Hold the pop so the loot reads clearly before flying to the cowboy.
	tween.tween_interval(HOLD_DURATION)
	tween.tween_property(_sprite, "scale", Vector2(1.1, 1.1), 0.12)
	tween.parallel().tween_property(_glow, "modulate:a", 0.35, 0.18)

	tween.tween_property(_sprite, "global_position", target_world + Vector2(0.0, -48.0), FLY_DURATION)\
		.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
	tween.parallel().tween_property(_sprite, "scale", Vector2(0.55, 0.55), FLY_DURATION)
	tween.parallel().tween_property(_sprite, "modulate:a", 0.55, FLY_DURATION)
	tween.parallel().tween_property(_sparkle, "modulate:a", 0.0, FLY_DURATION * 0.6)
